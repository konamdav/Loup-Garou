...
public class TurnBehaviour extends SimpleBehaviour {
	...
	public void action() {
		...
		/** etat d'attente de dÈbut de tour **/
		...
		/** etat envoi des requÈtes de reveil pour un joueur **/
		else if(this.step.equals(STATE_SEND_WAKE_ONE))
		{
			...
			if (agents.isEmpty() == false){
				...
				this.cpt_vol = 0 ; //Cpt to know if role init or delete of the vol√© et le voleur
...
//				/System.err.println("Launch voleur turn for " + this.current_volleur);
		...
		/** Demande de suppression role du voleur et du vol√©  **/
		...
		else if(this.step.equals(STATE_RECEIVE_DELETE_ROLE_REQUEST))
		{
			...
			if (this.cpt_vol == 2 )
			{
				...
				/*** reception role supprim√© du voleur**/
				MessageTemplate mt = MessageTemplate.and(
						MessageTemplate.MatchPerformative(ACLMessage.CONFIRM),
		...
		/** Reception des changements de r√¥le **/
		...
		/** etat envoi une requÈte de sommeil **/
		else if(this.step.equals(STATE_SEND_SLEEP_ONE))
		{
			ACLMessage messageRequest = new ACLMessage(ACLMessage.REQUEST);
			messageRequest.setSender(this.ctrlAgent.getAID());
