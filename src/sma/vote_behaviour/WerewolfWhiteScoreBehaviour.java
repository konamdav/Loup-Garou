...
//TODO CEDRIC
public class WerewolfWhiteScoreBehaviour extends Behaviour implements IVoteBehaviour {
	...
	public WerewolfWhiteScoreBehaviour(PlayerAgent playerAgent) {
		super();
		this.playerAgent = playerAgent;
		this.name_behaviour = "WEREWOLF_WHITE_SCORE";
	...
	public void action() {
		...
		else if(step.equals(STATE_SCORE))
		{
			...
			{
				scores.put(player.getName(), this.score(player, request, agents)); //TODO CEDRIC
			}
	...
	private int score(AID player,  VoteRequest request, List<AID> whitewerewolves)
	{
		...
		if(request.isVoteAgainst()){
			...
			else
			{
				...
				if(!isWhiteWerewolf || (isWhiteWerewolf  && localResults.getVoteCount(player.getName())!=0))
				{			
					// regles de scoring
					score += localResults.getVoteCount(player.getName(), whitewerewolves) *ScoreFactor.SCORE_FACTOR_WEREWOLF_VOTE;
					score += globalResults.getVoteCount(player.getName(), whitewerewolves) * ScoreFactor.SCORE_FACTOR_GLOBAL_VOTE;
					
					int diff =0;
					for(AID wolf : whitewerewolves)
					{
						diff+= localResults.getDifferenceVote(player.getName(), wolf.getName());
					}
					
					diff = diff/whitewerewolves.size();
