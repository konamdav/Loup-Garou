...
public class ExorcistInitBehaviour extends OneShotBehaviour{
	private PlayerAgent agent;
	private AID receiver;

	public ExorcistInitBehaviour(PlayerAgent agent, AID receiver) {
		super();
		this.agent = agent;
	...
	public void action() {
		...
		map_behaviour.put(Roles.EXORCIST, list_behav);
		
		System.out.println("[ "+this.agent.getName()+" ] REGISTER "+Roles.EXORCIST);
		DFServices.registerPlayerAgent(Roles.EXORCIST, this.myAgent, this.agent.getGameid());
		
		List<AID> agents = DFServices.findGameControllerAgent(Roles.EXORCIST, this.myAgent, this.agent.getGameid());
