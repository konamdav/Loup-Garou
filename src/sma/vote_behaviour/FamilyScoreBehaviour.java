 ...
 *
 */
public class FamilyScoreBehaviour extends Behaviour implements IVoteBehaviour{
	...
	public FamilyScoreBehaviour(PlayerAgent playerAgent) {
		super();
		this.playerAgent = playerAgent;
		this.name_behaviour = "FAMILY_SCORE";
	...
	private int score(AID player,  VoteRequest request, List<AID> family)
	{
		...
		if(request.isVoteAgainst()){
			...
			else
			{
				...
				if(!isFamily || (isFamily  && localResults.getVoteCount(player.getName())!=0))
				{			
					// regles de scoring
					score += localResults.getVoteCount(player.getName(), family) *ScoreFactor.SCORE_FACTOR_WEREWOLF_VOTE;
					score+= globalResults.getVoteCount(player.getName(), family) * ScoreFactor.SCORE_FACTOR_GLOBAL_VOTE;
					
					int diff =0;
					for(AID fluteplayer : family)
					{
